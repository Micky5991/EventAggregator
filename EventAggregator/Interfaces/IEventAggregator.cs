...
namespace Micky5991.EventAggregator.Interfaces
{
    public interface IEventAggregator
    {
        ...
        ///
        /// <see cref="PublishSync{T}"/> will NOT trigger a subscription created by this method!
        ///
        ...
        /// <summary>
        /// Subscribes the given callback to the <see cref="IEventAggregator"/>, so it will betriggered when
        /// <see cref="Publish{T}"/>, <see cref="PublishAsync{T}"/> or <see cref="PublishSync{T}"/> sends a new event.
        ///
        /// To listen to all events just subscribe to <see cref="IEvent"/>.
        /// </summary>
        /// <param name="eventCallback">Callback to be triggered when event <typeparam name="T"></typeparam> was published</param>
        /// <param name="filter">Predicate to choose if the callback should be triggered or not</param>
        /// <param name="priority">Priority of this subscription, Highest will be called last</param>
        /// <typeparam name="T">EventType to subscribe to</typeparam>
        /// <exception cref="ArgumentNullException"><param name="eventCallback"></param> is null</exception>
        /// <returns>Newly created subscription. Dispose to unsubscribe</returns>
        ISyncSubscription SubscribeSync<T>(EventAggregatorDelegates.EventCallback<T> eventCallback,
            EventAggregatorDelegates.EventFilter<T> filter = null, EventPriority priority = EventPriority.Normal) where T : IEvent;
        ...
        /// <summary>
        /// Publish event object and trigger all synchronous events.
        ///
        /// Subscriptions created with <see cref="Subscribe{T}"/> will NOT be called!
        /// </summary>
        /// <param name="eventData">Event data to send to all subscribers</param>
        /// <typeparam name="T">EventType that should be searched for and <see cref="IEvent"/>.</typeparam>
        /// <returns>Entered <param name="eventData"></param> to simplify usage</returns>
        T PublishSync<T>(T eventData) where T : IEvent;
